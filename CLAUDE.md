...
**이슈 단위 기능 개발은 반드시 아래 7단계 순서를 따른다.** Claude는 이 워크플로의 진행을 인식하고, 각 단계 완료 시 **다음 단계를 제안**하되 자동 실행하지 않는다.

| #   | 단계                                                             | 도구  | 산출물                                                            |
| --- | ---------------------------------------------------------------- | ----- | ----------------------------------------------------------------- |
| 1   | `/test-scenarios N`                                              | skill | 시그니처 + 테스트 시나리오 (`docs/features/<feature>/issue-N.md`) |
| 2   | `/tdd-red N`                                                     | skill | 실패하는 테스트                                                   |
| 3   | `/tdd-green N`                                                   | skill | 최소 구현, 테스트 전체 통과                                       |
| 4   | `@ac-verifier N`                                                 | agent | AC 충족 여부 독립 검증 (**테스트 통과 ≠ AC 충족**)                |
| 5   | `/tdd-refactor N`                                                | skill | 구조 개선, 깨지면 즉시 롤백                                       |
| 6   | `/security-review N`                                             | skill | 타입·보안 점검                                                    |
| 7   | commit → PR `--base feature/<spec>` → squash merge → 이슈 클로즈 | —     | 머지된 기능                                                       |

### 진행 규칙 (Claude가 흐름을 제어할 때)

- **각 단계 사이에는 인간 승인 게이트가 있다. 자동으로 다음 단계로 넘어가지 않는다.** 한 단계가 끝나면 결과를 보고하고 다음 단계를 제안한 뒤 사용자의 승인을 기다린다.
- **단계를 건너뛰지 않는다.** 사용자가 명시적으로 생략을 요청하지 않는 한 1→7 순서를 지킨다. 현재 어느 단계인지, 다음이 무엇인지 항상 알린다.
- **4단계(`@ac-verifier`)는 Green 직후 독립 검증**이다. 테스트가 모두 통과해도 AC 의도가 충족됐는지는 별도로 판단한다. (서브에이전트 스폰이 불가하면 동일 방법론으로 인라인 검증한다.)
- **5단계에서 테스트가 깨지면 고치지 말고 즉시 롤백**한다 (`git checkout -- <파일>`).
- **각 TDD 단계(2·3·5)는 끝나면 커밋**을 제안한다. Red/Green/Refactor를 별도 커밋으로 나눈다.
- **PR base는 항상 통합 `feature/<spec>` 브랜치**이며 `main`이 아니다. squash merge를 기본으로 한다.
- **이슈 의존성이 있으면 선행 이슈가 머지된 `feature/<spec>` 브랜치에서 분기**한다. 새 이슈 시작 시 base 브랜치가 최신인지(`git pull --ff-only`) 먼저 확인한다.
- PR base가 기본 브랜치가 아니면 머지 시 `Closes #N` 자동 클로즈가 동작하지 않으므로 **이슈를 수동으로 클로즈**한다.
